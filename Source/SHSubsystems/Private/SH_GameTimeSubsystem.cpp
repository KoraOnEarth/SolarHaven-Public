...
void USH_GameTimeSubsystem::IncrementTime()
{
	...
	// If the multiplier is, for example, 1,000,000, this function may become expensive.
...
void USH_GameTimeSubsystem::ProcessTimeEvents(const FSH_WorldTime& OldTime, const FSH_WorldTime& NewTime)
{
	...
	for (int32 t = OldTotal + 1; t <= NewTotal; t++)
	{
		...
		if (Minute == 0) OnHourChanged.Broadcast(CurrentTime);
		if (Minute == 0 && Hour == 0) OnDayChanged.Broadcast(CurrentTime);
		if (Minute == 0 && Hour == 0 && Day == 1) OnSeasonChanged.Broadcast(CurrentTime);
		if (Minute == 0 && Hour == 0 && Day == 1 && SeasonIndex == 0) OnYearChanged.Broadcast(CurrentTime);
	}
}

USH_GameTimeSubsystem::USH_GameTimeSubsystem()
{
	InitDefaultTime();
}
